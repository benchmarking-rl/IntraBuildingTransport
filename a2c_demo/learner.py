...
class Learner(object):
    def __init__(self, config):
        ...
        print ("action/obs dimention:", self._obs_dim, self._act_dim)
    ...
    def step(self):
        ...
        self.advantages = np.mean(train_batch['advantages'])
        self.target_values = np.mean(train_batch['target_values'])

        self.tf_log.log_scalar('total_loss', total_loss, self.sample_total_steps)
        self.tf_log.log_scalar('pi_loss', pi_loss, self.sample_total_steps)
        self.tf_log.log_scalar('vf_loss', vf_loss, self.sample_total_steps)
        self.tf_log.log_scalar('entropy', entropy, self.sample_total_steps)
        self.tf_log.log_scalar('lr', self.lr, self.sample_total_steps)
        self.tf_log.log_scalar('entropy_coeff', self.entropy_coeff, self.sample_total_steps)
        self.tf_log.log_scalar('advantages', self.advantages, self.sample_total_steps)
        self.tf_log.log_scalar('q_value', self.target_values, self.sample_total_steps)
    ...
    def log_metrics(self):
        ...
        episode_rewards, episode_steps, episode_origin_rewards = [], [], []
        for x in metrics:
            episode_rewards.extend(x['episode_rewards'])
            episode_origin_rewards.extend(x['episode_origin_rewards'])
        ...
        max_episode_origin_rewards, mean_episode_origin_rewards, min_episode_origin_rewards = \
                None, None, None
        if episode_rewards:
            ...
            mean_episode_origin_rewards = np.mean(np.array(episode_origin_rewards).flatten())
            max_episode_origin_rewards = np.max(np.array(episode_origin_rewards).flatten())
            min_episode_origin_rewards = np.min(np.array(episode_origin_rewards).flatten())
        ...
        metric = {
            ...
            'max_episode_origin_rewards': max_episode_origin_rewards,
            'mean_episode_origin_rewards': mean_episode_origin_rewards,
            'min_episode_origin_rewards': min_episode_origin_rewards,
        ...
        self.tf_log.log_scalar('max_episode_rewards', max_episode_rewards, self.sample_total_steps)
        self.tf_log.log_scalar('mean_episode_rewards', mean_episode_rewards, self.sample_total_steps)
        self.tf_log.log_scalar('min_episode_rewards', min_episode_rewards, self.sample_total_steps)
 
        self.tf_log.log_scalar('max_episode_origin_rewards', max_episode_origin_rewards, self.sample_total_steps)
        self.tf_log.log_scalar('mean_episode_origin_rewards', mean_episode_origin_rewards, self.sample_total_steps)
        self.tf_log.log_scalar('min_episode_origin_rewards', min_episode_origin_rewards, self.sample_total_steps)
